...
export default createScaffolding({
  name: '~common/logging/otel',
  async init({ modules }, { addRequest, addExecutor, setStatus }) {
    const config = modules['~common/logging']?.config;
    const enabled = ['highlight', 'grafana'].includes(config?.provider);
    await addRequest({
      match: 'tsconfig:exclude',
      value: { path: 'src/common/logging/otel', excluded: !enabled, context: '[all]' },
    });
    if (!enabled) {
      setStatus('disabled');
      return;
    ...
    await addRequest({
      ...
      value: {
        module: 'AppModule',
        fileImports: [['OtelModule', '~common/logging/otel/otel.module']],
        imports: ['OtelModule'],
      },
    });
