...
export default createScaffolding({
  name: '~common/logging/sentry',

  async init({ modules }, { addRequest, setStatus }) {
    const provider = modules['~common/logging']?.config?.provider;

    await addRequest({
      match: 'tsconfig:exclude',
      value: { path: 'src/common/logging/sentry', excluded: provider !== 'sentry', context: '[all]' },
    });
