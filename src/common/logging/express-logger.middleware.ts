...
import { makeId } from '~vendors/short-uuid';

import { getConfig } from '~common/config';
import { HttpConfig } from '~common/http/http.config';
import type { IHttpRequestLog, IHttpResponseLog } from '~common/http/interfaces';
import { LoggerService } from '~common/logging';
import { type ILoggerStore, loggerStore } from '~common/logging/logger.store';
...
export function ExpressLoggerMiddleware(request: Request, response: Response, next: NextFunction) {
  ...
  if (httpConfig.log) {
    ...
    response.on('finish', async () => {
      if (request.userId) store.userId = request.userId;
      httpLogger.send({
        ...log,
        type: 'http',
        level: 'log',
